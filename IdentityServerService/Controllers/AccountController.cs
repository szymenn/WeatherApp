...
//                        return Redirect(model.ReturnUrl);
        ...
        [HttpGet("[controller]/Register", Name = "Register")]
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Register(RegisterBindingModel model, string returnUrl)
        {
            if (!ModelState.IsValid)
            {
                return View();
            ...
            if (result.Succeeded)
            {
                var loginModel = await BuildLoginViewModelAsync(model.ReturnUrl);   
                return RedirectToAction(nameof(Login), loginModel);
            }
            ModelState.AddModelError("Registration error", "Please check input data");
            return View();
        }
